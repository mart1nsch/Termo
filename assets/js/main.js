...
function getSecretWord(ac) {
    try {
        if (ac >= 200) {
            console.log(`Timeout no consumo`);
            return;
        }
        const request = new XMLHttpRequest();
        request.open(`GET`, 'https://api.dicionario-aberto.net/random', false);
        request.send();
        
        if (request.status === 200) {
            const response = JSON.parse(request.responseText);
            
            if (response.word.length === 5 && verificaAcentos(response.word)) {
                
                return response.word;
            } else {
                return getSecretWord(ac++);
            }
        } else {
